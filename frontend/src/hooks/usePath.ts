...
export const usePath = () => {
  ...
  const handlePathChange = (
    ...
    // 强制刷新时清除缓存，强制从服务器获取最新数据
    if (force) {
      log(`handle [${getHistoryKey(path, index)}] force refresh`)
      clearHistory(path, index)
      delete IsDirRecord[path]
    ...
    if (hasHistory(path, index)) {
      ...
      return handleFolder(path, index, undefined, undefined, undefined, force)
    } else {
